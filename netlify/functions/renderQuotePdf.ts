...
function docToBuffer(doc: any) {
  return new Promise<Buffer>((resolve, reject) => {
    ...
    doc.on("error", reject);
    // IMPORTANTE: No llamar a doc.end() aquí, se llama al final del flujo principal
  });
}
...
function drawWatermark(doc: any, wmBuf: Buffer | null) {
  ...
  doc.save();
  doc.opacity(0.05); // ESTO ES LO QUE ESTABA ROMPIENDO EL DISEÑO SI NO SE RESTAURA
  try {
    doc.image(wmBuf, x, y, { width: w });
  } catch {
    // ignore
  }
  doc.restore(); // RESTAURA LA OPACIDAD A 1.0 PARA EL RESTO DEL PDF
}
...
function drawFooter(doc: any, lang: "es" | "en") {
  ...
  doc.save();
  // Intentamos usar Inter, si falla Helvetica (Seguridad)
...
function drawHeader(
  ...
  doc.fontSize(16).text("Fresh Food Panamá", x + 120, topY + 2);

  doc.fillColor("#475569");
  try { doc.font("Inter"); } catch { doc.font("Helvetica"); }
  doc.fontSize(10).text(`${t(lang, "Cotización", "Quotation")} ${quoteNumber}`, x + 120, topY + 22);
...
function drawItemsTable(doc: any, opts: { lang: "es" | "en"; currency: string; items: any[]; total: number; boxWidth: number }) {
  ...
  doc.save();
  doc.opacity(0.04).rect(x, tableTopY, boxWidth, headerH).fill("#000");
  doc.restore();
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    if (!user || !profile || !isPrivileged(profile.role)) return text(401, "Forbidden");
    ...
    // Lógica de Variables (Tu original)
    ...
    // REGISTRO DE FUENTES (Antes de empezar a dibujar)
    if (interRegularBuf) doc.registerFont("Inter", interRegularBuf);
    if (interBoldBuf) doc.registerFont("Inter-Bold", interBoldBuf);
    doc.font("Inter");

    const boxWidth = doc.page.width - doc.page.margins.left - doc.page.margins.right;
    const pBufferPromise = docToBuffer(doc);
    ...
    if (variant === "1") {
      ...
      drawKeyValueLines(doc, [{ k: "Total", v: money(total, currency) }], boxWidth);
    } else {
    ...
    doc.end(); // CERRAMOS MANUALMENTE
    const pdfBuffer = await pBufferPromise;

    return {
      statusCode: 200,
      headers: { 
          "Content-Type": "application/pdf",
          "Content-Disposition": `attachment; filename="${safeFileName(clientName)}.pdf"`
      },
